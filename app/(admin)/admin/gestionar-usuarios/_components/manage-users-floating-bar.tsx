...
interface ManageUsersFloatingBarProps {
  table: Table<User>;
}

export function ManageUsersFloatingBar({ table }: ManageUsersFloatingBarProps) {
  const rows = table.getFilteredSelectedRowModel().rows;

  const [isPending, startTransition] = useTransition();
  const { mutate: deleteUsers, isPending: isDeletePending } = useDeleteUsers();
  const [action, setAction] = useState<"export" | "delete" | "update-role">();
