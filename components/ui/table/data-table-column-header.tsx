...
export function DataTableColumnHeader<TData, TValue>({
  ...
  const resetSort = `${column.id}-reset-sort`;
    ...
    <div className={cn("flex items-center gap-2", className)}>
      <Select
        ...
        onValueChange={(value) => {
          ...
          else if (value === resetSort) column.clearSorting();
        }}
      >
        <SelectTrigger
          aria-label={
            column.getIsSorted() === "desc"
              ? "Ordenado descendentemente. Click para ordenar ascendentemente."
              : column.getIsSorted() === "asc"
                ? "Ordenado ascendentemente. Click para ordenar descendentemente."
                : "Sin ordenar. Click para ordenar ascendentemente."
          }
        ...
        >
          ...
          <SelectIcon asChild>
            {column.getCanSort() && column.getIsSorted() === "desc" ? (
              <IconArrowDown className="ml-2.5 size-4" aria-hidden="true" />
            ) : column.getIsSorted() === "asc" ? (
              <IconArrowUp className="ml-2.5 size-4" aria-hidden="true" />
            ) : (
              <IconSelector className="ml-2.5 size-4" aria-hidden="true" />
            )}
          </SelectIcon>
        </SelectTrigger>
